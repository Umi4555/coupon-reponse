...
export class LoginUI {
            ...
            <div class="admin-dashboard-container">
                <div class="admin-header">
                    <div class="back-to-home-container">
                        <a href="https://umi4555.github.io/accueil/" class="btn btn-back-home">
                            ← Retour à l'accueil
                        </a>
                    </div>
                    <div class="logo-container">
                        <img src="logo (1).png" alt="Logo Lycée Château Blanc" class="lycee-logo">
    ...
    createFiliereItem(filiere) {
        ...
        // Remove duplicates
        const uniqueDocuments = Array.from(new Set(documents.map(JSON.stringify)))
            .map(JSON.parse);
        ...
        return `
            <div class="category-item" data-filiere="${filiere}">
                <div class="filiere-header">
                    <h3>${fullName}</h3>
                    <div class="document-count">${uniqueDocuments.length} documents</div>
                </div>
                ...
                ${this.generateDocumentList(uniqueDocuments)}
            </div>
        `;
    }

    generateDocumentList(documents) {
        // Ensure documents is an array and remove any potential duplicates
        const uniqueDocuments = Array.from(new Set(documents.map(JSON.stringify)))
            .map(JSON.parse);

        // Check if there are actually no documents
